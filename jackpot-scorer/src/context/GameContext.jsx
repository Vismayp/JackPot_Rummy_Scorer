...
function gameReducer(state, action) {
  switch (action.type) {
    ...
    case 'REENTER_PLAYER': {
      ...
      const lowestScore = getLowestActiveScore(state.players);

      return {
        ...state,
        players: state.players.map(p =>
          p.id === playerId
            ? { ...p, status: 'active', totalScore: lowestScore, hasReentered: true }
            : p
        )
      };
    }
    ...
    case 'UNDO_ROUND': {
      ...
      const newPlayers = state.players.map(player => {
        ...
        const wasEliminatedThisRound = 
          ...
          (player.totalScore >= ELIMINATION_THRESHOLD && newTotal < ELIMINATION_THRESHOLD);
