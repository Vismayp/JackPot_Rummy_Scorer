...
export const shouldEliminate = (totalScore) =>
  totalScore >= ELIMINATION_THRESHOLD;
...
// Get highest score among non-eliminated players (for re-entry)
export const getHighestActiveScore = (players) => {
  const activePlayers = players.filter((p) => p.status === "active");
  if (activePlayers.length === 0) return 0;
  return Math.max(...activePlayers.map((p) => p.totalScore));
};
