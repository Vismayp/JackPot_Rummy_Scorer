...
export default function Leaderboard() {
  ...
  const isGameOver =
    (activePlayers.length <= 1 && state.rounds.length > 0) ||
    (isSecretSeven && state.rounds.length >= 7);
    ...
    <div className="leaderboard">
      ...
      <div className="leaderboard-list">
        {sortedPlayers.map((player, index) => (
          ...
          >
              ...
              {player.totalScore >= 200 && player.status === "active" && (
                <div className="warning-bar">
                  <div
                    ...
                    style={{
                      width: `${Math.min(100, (player.totalScore / ELIMINATION_THRESHOLD) * 100)}%`,
                    }}
                  />
                </div>
              )}
            ...
            <div className="player-total">{player.totalScore}</div>
      ...
      {isGameOver && (
        <div className="game-over">
          ...
          {(activePlayers.length === 1 || isSecretSeven) && (
            <span className="winner">
              WINNER: {sortedPlayers.find((p) => p.status === "active")?.name}
            </span>
          )}
        </div>
      )}
